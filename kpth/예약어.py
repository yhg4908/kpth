한국어_파이썬_예약어 = {
    ...
    "함께": "with",
    "생성": "yield",
    "모두생성": "yield from", # yield from

    # 컴프리헨션 관련
    "리스트컴프리헨션": "[x for x in iterable]",  # 설명용
    "사전컴프리헨션": "{k:v for k,v in items}",   # 설명용
    "집합컴프리헨션": "{x for x in iterable}",    # 설명용
    "제너레이터컴프리헨션": "(x for x in iterable)", # 설명용

    # 특수 연산자
    "왈러스": ":=",          # 왈러스 연산자 (Python 3.8+)
    "언패킹": "*",           # 시퀀스 언패킹
    "사전언패킹": "**",       # 사전 언패킹
